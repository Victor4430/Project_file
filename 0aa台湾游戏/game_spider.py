...
def run():
    ...
    sheet.cell(row=1, column=3).value = a2
    sheet.cell(row=1, column=5).value = a3
    sheet.cell(row=1, column=7).value = a4
    sheet.cell(row=1, column=9).value = a5
    sheet.cell(row=1, column=11).value = a6
        ...
        if a1 in i:
            ...
            for j in range(1, r+1):
                ...
                if sheet.cell(row=j, column=1).value != None:
                    ...
                    # 如果本行 帳號 字段为空
                    # 则将 此次字段内容写入
                    sheet.cell(row=j, column=1).value = i1
                    sheet.cell(row=j, column=2).value = i2
                    # 跳出此次循环
                    break
                # 以下循环同理
        elif a2 in i:
            ...
            for j in range(1, r + 1):
                if sheet.cell(row=j, column=3).value != None:
                    ...
                    sheet.cell(row=j, column=3).value = i1
                    sheet.cell(row=j, column=4).value = i2
                    break
        elif a3 in i:
            r += 1
            for j in range(1, r + 1):
                if sheet.cell(row=j, column=5).value != None:
                    ...
                    sheet.cell(row=j, column=5).value = i1
                    sheet.cell(row=j, column=6).value = i2
                    break
        elif a4 in i:
            r += 1
            for j in range(1, r + 1):
                if sheet.cell(row=j, column=7).value != None:
                    continue
                else:
                    sheet.cell(row=j, column=7).value = i1
                    sheet.cell(row=j, column=8).value = i2
                    break
        elif a5 in i:
            r += 1
            for j in range(1, r + 1):
                if sheet.cell(row=j, column=9).value != None:
                    ...
                    sheet.cell(row=j, column=9).value = i1
                    sheet.cell(row=j, column=10).value = i2
                    break
        elif a6 in i:
            r += 1
            for j in range(1, r + 1):
                if sheet.cell(row=j, column=11).value != None:
                    continue
                else:
                    sheet.cell(row=j, column=11).value = i1
                    sheet.cell(row=j, column=12).value = i2
                    break
