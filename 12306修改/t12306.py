...
class TICKETPUACH(object):
    def __init__(self, auto_search_times=10, auto_search_fre=5, auto_search_endtime=None,
                 ...
                 date=datetime.datetime.now().strftime('%Y-%m-%d'), train_rule="name", timearea=["00:30", "23:30"],
                 username="156344351", passwd="13543513", passengers=[]) -> None:
        # 自动刷新次数
    ...
    def search(self):
                ...
                for tr in trs:
                    ...
                    for td in tds:
                        ...
                        if len(find_base_info) > 0:
                            # 基础信息
                            t_value.append(
                                find_base_info[0].query_selector_all('//div[@class="train"]')[0].inner_text().replace(
                                    "\n", "-"))
                            t_value.append(
                                find_base_info[0].query_selector_all('//div[@class="cdz"]')[0].inner_text().replace(
                                    "\n", "-"))
                            t_value.append(
                                find_base_info[0].query_selector_all('//div[@class="cds"]')[0].inner_text().replace(
                                    "\n", "-"))
                            t_value.append(
                                find_base_info[0].query_selector_all('//div[@class="ls"]')[0].inner_text().replace("\n",
                                                                                                                   "-"))
        ...
        # ---------------------
        # 订单信息有了，出票中，修正code
        if data["results"]["order"] != "" and len(data["results"]["passagers"]) > 0 and data["results"]["passagers"][0][
            "票价"] == "":
            code = 510
                                ...
                                if len(tr_passager_by_orders) > 0:
                                    for tr_passager_by_order in tr_passager_by_orders:
                                        ...
                                        if td_passager_by_orders[0].get_attribute("class") == "td-left align-top":
                                            ...
                                            pas_info_value[1] = \
                                            td_passager_by_orders[1].query_selector_all("div >> div")[0].inner_text()
                                            ...
                                            pas_info_value = ['1', '', '中国居民身份证', '', '', '', '', '', '']
                                            pas_info_value[1] = \
                                            td_passager_by_orders[0].query_selector_all("div >> div")[0].inner_text()
