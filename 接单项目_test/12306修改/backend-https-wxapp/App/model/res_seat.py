...
#类
class RES_SEAT(db.Model):
    #用户表名
    __tablename__='res_seat'
    #id，主键，采用uuid
    id=db.Column(VARCHAR,primary_key=True)
    task_id=db.Column(VARCHAR)
    code=db.Column(INTEGER)
    msg=db.Column(VARCHAR)
    train_info=db.Column(JSONB)
    passagers=db.Column(JSONB)
    train_name=db.Column(VARCHAR)
    seat_type=db.Column(VARCHAR)
    order=db.Column(VARCHAR)
