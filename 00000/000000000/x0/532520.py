...
def run():
    ...
    for j in range(1, 1000000000):
        ...
        url_list_0 = html.xpath('//*[@id="app"]/div[4]/div[1]/a/@href')

        print(url_list_0, '访问成功！')
        url_join = 'http://360308.com'
        for i in url_list_0:

            a = url_join + i
            # print(a)
            req_2 = requests.get(a, headers=headers, proxies=proxies).text
            html_2 = etree.HTML(req_2)
            url_list_1 = html_2.xpath('//*[@id="app"]/div[2]/div[1]/div/iframe/@src')[0]
            with open('./m3u8_url.txt', 'a') as f:
                print(url_list_1)
                f.write(url_list_1 + '\n')
                print('写入成功.................第 {} 条...'.format(j + b))
            b += 1

        next_url = 'http://360308.com' + html.xpath('//*[@id="app"]/div[4]/div[2]/a[1]/@href')[0]
        req = requests.get(next_url, headers=headers, proxies=proxies)
        req.encoding = 'utf-8'
        # print(req.text)
